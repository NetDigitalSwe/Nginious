...
import com.nginious.http.application.ControllerServiceFactory;
import com.nginious.http.server.HttpTestRequest;
import com.nginious.http.server.HttpTestResponse;

public class RestControllerSerializationTestCase extends XMLTestCase {
	
	public RestControllerSerializationTestCase() {
		super();
	}

	public RestControllerSerializationTestCase(String name) {
		super(name);
	}
	...
	public void testRestControllerSerialization() throws Exception {
		...
		ControllerService invoke = factory.createControllerService(service);
	...
	public void testInvalidAcceptSerialization() throws Exception {
		...
		try {
			invoke.invoke(request, response);
			fail("Must not be possible to invoke with bad accept type");
		} catch(HttpException e) {
			assertEquals(HttpStatus.BAD_REQUEST, e.getStatus());
		}		
	}
	...
	public static Test suite() {
		return new TestSuite(RestControllerSerializationTestCase.class);
	}
