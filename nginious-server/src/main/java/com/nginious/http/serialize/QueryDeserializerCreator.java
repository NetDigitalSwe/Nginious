...
class QueryDeserializerCreator extends DeserializerCreator<QueryDeserializer<?>> {
	...
	/**
	 * Constructs a new query deserializer creator with the specified class loader.
	 * 
	 * @param classLoader the class loader to use for loading created deserializer classes
	 */
	QueryDeserializerCreator(ApplicationClassLoader classLoader) {
		super();
	...
	protected <T> QueryDeserializer<T> create(Class<T> beanClazz) throws SerializerFactoryException {
		...
		try {
			synchronized(this) {
				...
				String signature = Serialization.createClassSignature("com/nginious/serialize/QueryDeserializer", intBeanClazzName);
				writer.visit(Opcodes.V1_6, Opcodes.ACC_PUBLIC, intDeserializerClazzName, signature, "com/nginious/http/serialize/QueryDeserializer", null);
