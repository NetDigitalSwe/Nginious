...
class JsonDeserializerCreator extends DeserializerCreator<JsonDeserializer<?>> {
	...
	/**
	 * Constructs a new JSON deserializer creator.
	 */
	JsonDeserializerCreator() {
		super();
	}
	...
	protected <T> JsonDeserializer<T> create(Class<T> beanClazz) throws SerializerFactoryException {
		...
		try {
			synchronized(this) {
				...
				String signature = Serialization.createClassSignature("com/nginious/http/rest/JsonDeserializer", intBeanClazzName);
				writer.visit(Opcodes.V1_6, Opcodes.ACC_PUBLIC, intDeserializerClazzName, signature, "com/nginious/http/rest/JsonDeserializer", null);
				
				// Create constructor
				Serialization.createConstructor(writer, "com/nginious/http/rest/JsonDeserializer");
