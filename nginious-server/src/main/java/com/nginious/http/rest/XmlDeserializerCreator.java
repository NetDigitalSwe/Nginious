...
class XmlDeserializerCreator extends DeserializerCreator<XmlDeserializer<?>> {
	...
	protected <T> XmlDeserializer<T> create(Class<T> beanClazz) throws SerializerFactoryException {
		...
		try {
			synchronized(this) {
				...
				String signature = Serialization.createClassSignature("com/nginious/http/rest/XmlDeserializer", intBeanClazzName);
				writer.visit(Opcodes.V1_6, Opcodes.ACC_PUBLIC, intDeserializerClazzName, signature, "com/nginious/http/rest/XmlDeserializer", null);
				
				// Create constructor
				Serialization.createConstructor(writer, "com/nginious/http/rest/XmlDeserializer");
	...
	private MethodVisitor createDeserializeMethod(ClassWriter writer, String intBeanClazzName) {
		String[] exceptions = { "com/nginious/http/rest/SerializerException" };			
