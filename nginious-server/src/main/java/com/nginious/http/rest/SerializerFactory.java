...
public class SerializerFactory {
	...
	private JsonSerializerCreator jsonCreator = new JsonSerializerCreator();
	
	private XmlSerializerCreator xmlCreator = new XmlSerializerCreator();
	...
	/**
	 * Constructs a new serializer factory.
	 */
	SerializerFactory() {
		super();
		this.jsonCreator = new JsonSerializerCreator();
		this.xmlCreator = new XmlSerializerCreator();
	}
		...
		try {
			...
			for(HeaderParameter parameter : sorted) {
				if(parameter.accepts("application/json")) {
					serializer = jsonCreator.create(beanClazz);
				} else if(parameter.accepts("text/xml") || parameter.accepts("application/xml")) {
					serializer = xmlCreator.create(beanClazz);
				}				
	 ...
	 */
	public static <T> JsonSerializer<T> createJsonSerializer(Class<T> beanClazz) throws SerializerFactoryException {
		return getInstance().jsonCreator.create(beanClazz);
	}
	 ...
	 * @throws SerializerFactoryException if class is not a bean
	 */
	public static <T> XmlSerializer<T> createXmlSerializer(Class<T> beanClazz) throws SerializerFactoryException {
		return getInstance().xmlCreator.create(beanClazz);		
	}
